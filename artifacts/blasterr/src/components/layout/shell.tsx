...
export function Shell({ children }: ShellProps) {
  ...
  if (location === "/" || location === "/sign-in" || location === "/sign-up") {
    return <>{children}</>;
  }
          ...
          {user ? (
            <>
              <Link href="/create" className="w-full">
                <Button size="lg" className="w-full h-14 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-display font-bold text-xl shadow-[0_0_20px_rgba(204,255,0,0.4)] transition-all hover:shadow-[0_0_30px_rgba(204,255,0,0.6)] hover:scale-[1.02]">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 glass-panel border-t border-white/10 z-50 px-6 py-3 flex justify-between items-center safe-area-bottom">
        ...
        {user && (
          <Link href="/create" className="absolute -top-6 left-1/2 -translate-x-1/2">
            <Button size="icon" className="w-14 h-14 rounded-full bg-primary text-primary-foreground shadow-[0_0_20px_rgba(204,255,0,0.4)]">
